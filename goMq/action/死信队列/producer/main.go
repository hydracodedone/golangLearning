...
func main() {
    ...
    //创建发布交换机
    channel.ExchangeDeclare(normalExchangeName, exchangeType, durable, autoDelete, normalExchangeInternal, noWait)
    //创建死信交换机
    channel.ExchangeDeclare(deadExchageName, exchangeType, durable, autoDelete, deadExchangeInternal, noWait)
    //创建队列
    //死信配置
    ...
    endloopSelct:
        for {
            select {
            case ackReturn, ok := <-channel.NotifyReturn:
                if !ok {
