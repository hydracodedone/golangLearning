...
// ConsumeWithAllTopicPartitions 消费指定主题的所有分区（并发消费每个分区）
func ConsumeWithAllTopicPartitions(ctx context.Context, topic string) error {
    ...
    wg := &sync.WaitGroup{}
    for _, eachPartition := range partitions {
        partitionConsumer := getPartitionConsumer(consumer, topic, eachPartition, sarama.OffsetOldest)
        if partitionConsumer == nil {
            err := errors.New("nil partition consumer")
            return err
        }
        startConsumerErrorChanListen(wg, partitionConsumer)
        go partitionConsumerConsume(ctx, wg, partitionConsumer)
    }
    wg.Wait()
    return nil
}
