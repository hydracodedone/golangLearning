...
const MySQLDSN = "root:123456@(47.120.68.104:3306)/test?charset=utf8mb4&parseTime=True&loc=Local"
...
func CodeGenerate() {
	...
	// 创建有关联关系的模型文件
	uidForeignKeyTag := map[string][]string{
		"foreignKey": {"UID"},
	}
	User := g.GenerateModel("user",
		append(
			...
			gen.FieldRelate(field.HasMany, "Address", Address, &field.RelateConfig{GORMTag: uidForeignKeyTag}),
		)...,
	)
	Address = g.GenerateModel("address",
		append(
			fieldOpts,
			gen.FieldRelate(field.BelongsTo, "User", User, &field.RelateConfig{GORMTag: uidForeignKeyTag}),
		)...,
	)
...
func main() {
	...
	defer cancelFunc()
	// err = dal.User.WithContext(ctx).Create(&model.User{
	// 	Name: "Hydra",
	// 	Age:  18,
	// })
	// if err != nil {
	// 	panic(err)
	// }
	userModel, err := dal.User.WithContext(ctx).Where(dal.User.Age.Eq(18)).Find()
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", userModel[0])
