...
### slice扩容规律
   ...
   切片的扩容流程源码位于 runtime/slice.go 文件的 growslice 方法当中
    ...
    3.如果发生扩容,则扩容后会重新分配内存和拷贝数据,底层的地址会发生变化
```Go
...
名词解释:
    ...
    复用线程
        ...
        hand off机制
            当本M因为G进行系统调用阻塞时,M释放绑定的P,唤醒休眠的M,并绑定该P(没有唤醒的就创建新的M再去绑定)执行。需要注意,不会区操作自旋M,是因为自旋的M已经绑定了P
    利用并行
...
调度策略具体说明:
    ...
    3.在创建G时，运行的G会尝试唤醒其他空闲的P和M组合(创建的G不会被u新的P和M执行)
